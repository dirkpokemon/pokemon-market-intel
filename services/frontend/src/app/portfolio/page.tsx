      ...
      <div className="px-6 py-8 max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p className="text-2xl font-bold text-gray-900">&euro;{portfolioStats.totalValue.toFixed(2)}</p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p className={`text-2xl font-bold ${portfolioStats.totalPL >= 0 ? 'text-green-700' : 'text-red-700'}`}>
              {portfolioStats.totalPL >= 0 ? '+' : ''}&euro;{portfolioStats.totalPL.toFixed(2)}
            </p>
            <p className={`text-[11px] mt-0.5 font-medium ${portfolioStats.plPct >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {portfolioStats.plPct >= 0 ? '+' : ''}{portfolioStats.plPct.toFixed(1)}%
            </p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <p className="text-xs text-gray-500 mb-1">Watchlist</p>
            <p className="text-2xl font-bold text-gray-900">{portfolioStats.watchlistCount}</p>
            {portfolioStats.triggeredAlerts > 0 && (
              <p className="text-[11px] mt-0.5 font-semibold text-green-600">{portfolioStats.triggeredAlerts} alert{portfolioStats.triggeredAlerts !== 1 ? 's' : ''} triggered!</p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center border-b border-gray-200 mb-6">
          ...
          >
            My Collection
            <span className="ml-2 text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">{collection.length}</span>
          </button>
          <button
          ...
          >
            My Watchlist
            <span className="ml-2 text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">{watchlist.length}</span>
            {portfolioStats.triggeredAlerts > 0 && (
          ...
          <>
            ...
            {showAddCard && (
              <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
                <h3 className="text-sm font-semibold text-gray-900 mb-4">Add a card to your collection</h3>
                
                {/* Search to find card */}
                <div className="relative mb-4">
                  <label className="block text-xs text-gray-500 mb-1.5">Search card</label>
                  <input
                    type="text"
                    value={addCardSearch}
                    onChange={(e) => { setAddCardSearch(e.target.value); searchForCard(e.target.value); }}
                    placeholder="Type to search (e.g. Charizard ex)..."
                    className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-gray-400 focus:border-transparent"
                  />
                  {addCardLoading && (
                      ...
                      {addCardResults.map((r, i) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">{r.card_name}</p>
                            {r.card_set && <p className="text-[11px] text-gray-400 truncate">{r.card_set}</p>}
                          </div>
                          <span className="text-sm font-semibold text-gray-700">&euro;{r.min_price.toFixed(2)}</span>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-1.5">Set</label>
                    <input
                      type="text"
                      value={newCard.set}
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-1.5">Purchase price (&euro;)</label>
                    <input
                      type="number"
                ...
                <div className="flex justify-end gap-3 mt-5 pt-4 border-t border-gray-100">
                  <button onClick={() => { setShowAddCard(false); setAddCardResults([]); }} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">
                    Cancel
                  </button>
                  <button
            ...
            {collection.length === 0 && !showAddCard ? (
              <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                ...
                <p className="text-sm font-medium text-gray-900 mb-1">Your collection is empty</p>
                <p className="text-xs text-gray-500 mb-5">Add cards you own to track their value over time.</p>
                <button
              ...
              <div className="space-y-2">
                {enrichedCollection.map((card) => (
                  <div key={card.id} className="bg-white rounded-xl border border-gray-200 p-4 hover:border-gray-300 transition">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <span className="text-[10px] px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded font-medium">{card.condition}</span>
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <p className="text-sm font-medium text-gray-600">&euro;{card.purchasePrice.toFixed(2)}</p>
                      </div>

                      {/* Current value */}
                      <div className="text-right flex-shrink-0">
                        <p className="text-[11px] text-gray-400">Current</p>
                        <p className="text-sm font-bold text-gray-900">&euro;{card.currentPrice.toFixed(2)}</p>
                      </div>

                      {/* P&L */}
                      <div className="text-right flex-shrink-0 min-w-[80px]">
                        <p className="text-[11px] text-gray-400">P&amp;L</p>
                        <p className={`text-sm font-bold ${card.profitLoss >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                          {card.profitLoss >= 0 ? '+' : ''}&euro;{card.profitLoss.toFixed(2)}
                        </p>
                        <p className={`text-[10px] font-medium ${card.profitLossPct >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      ...
                      {card.dealScore && (
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${
                          card.dealScore >= 80 ? 'bg-green-50' : card.dealScore >= 65 ? 'bg-amber-50' : 'bg-gray-50'
                        }`}>
                          <span className={`text-sm font-bold ${
                            card.dealScore >= 80 ? 'text-green-700' : card.dealScore >= 65 ? 'text-amber-700' : 'text-gray-600'
                          }`}>{card.dealScore}</span>
                        </div>
                      )}
                      ...
                      <button
                        ...
                        className="p-1.5 text-gray-300 hover:text-red-500 transition rounded flex-shrink-0"
        ...
        {activeTab === 'watchlist' && (
          <>
            ...
            {showAddWatch && (
              <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
                <h3 className="text-sm font-semibold text-gray-900 mb-1">Set a price target</h3>
                <p className="text-xs text-gray-500 mb-4">Get notified when the card reaches your target price</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-1.5">Target price (&euro;)</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-1.5">Notify me when</label>
                ...
                <div className="flex justify-end gap-3 mt-5 pt-4 border-t border-gray-100">
                  <button onClick={() => { setShowAddWatch(false); setWatchSearchResults([]); }} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">
                    Cancel
                  </button>
                  <button
            ...
            {watchlist.length === 0 && !showAddWatch ? (
              <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                ...
                <p className="text-xs text-gray-500 mb-5">Watch cards you want to buy and set target prices to get notified.</p>
                <button
                ...
                {enrichedWatchlist.map((card) => (
                  <div
                    ...
                    className={`bg-white rounded-xl border p-4 transition ${
                      card.triggered ? 'border-green-200 bg-green-50/30' : 'border-gray-200 hover:border-gray-300'
                    }`}
                  >
                    <div className="flex items-center gap-4">
                      {/* Status icon */}
